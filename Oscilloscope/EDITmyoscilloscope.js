...
|                                                         |
| Share â€” copy and redistribute the material in any       |
|         medium or format                                |
...
|                                                         |
| Attribution â€” You must give appropriate credit, include |
...
function Oscilloscope(id, userSettings){
	...
	this.timeDomain = new Uint8Array(this.analyser.frequencyBinCount);  //create an array of ints size = frequencyBinCount - buffer length
	this.streaming = false;
	this.sampleRate = this.audioContext.sampleRate;  // get sample rate from audioContext - samples per second used by all nodes
	this.numSamples = this.analyser.frequencyBinCount; // fbc is number of data values = numSamples

	this.fData = new Uint8Array(this.analyser.frequencyBinCount);  //ADDED 3/14
	var binSize = (this.sampleRate*0.5) / this.analyser.frequencyBinCount;
	//console.log("Bin Size: " + binSize + " Hz"); //5.859375 Hz
		...
		$("#ui-container-osc input, #ui-container-osc select").on("change", function(){
			...
			if ($(this).attr("name") == "freezeValue"){
				if ($(this).prop("checked") == true){
                    ...
                    document.getElementById('freeze-value').checked = false; //********* 
	...
	this.insertStyles = function(){
        //var incLeftPadding = this.padding.left + 20;
		var style = "<style>";
		//style += this.selector + " {position:relative;background: #5db1a2; width: ";
		style += this.selector + " {position:relative;background: #5db1a2; width: ";
		style += (this.c3.width + this.padding.right + this.padding.left); //originally c2.width
		style += "px;height:";
		...
		style += this.selector + " canvas{top:" + this.padding.top + "px;left:" + (this.padding.left + 20) + "px}";  //added 20
	...
	this.setupCanvases = function(){
		...
		this.c.height = 200;
		this.c2.width = 500;
		this.c3.width = this.c2.width + 70; // needs 50 so labels don't get cut off (originally 55)
		this.c2.height = 262;
		this.c2.height = 200;
  ...
  **/
	...
	this.createGrid = function(){
		...
		this.ctx3.fillText("Oscilloscope", 2*this.c3.width/5, 30); //******* originally this.c3.width/2, 20 */
        
		this.ctx3.font = "12px Arial";
		this.ctx3.fillText("Dominant", 3*this.c3.width/5, 10); 
        this.ctx3.fillText("Frequency:", 3*this.c3.width/5, 30); 
		//this.ctx3.fillText("Note: ", 4*this.c3.width/5 + 40, 30);
		

		this.ctx3.font = "20px Arial";
		this.ctx3.fillText("400 Hz", 3*this.c3.width/5 + 80, 30);
        // Adding X/Y Labels  ****************************

		this.ctx3.font = "14px Arial";

        this.ctx3.fillText("Time (ms)", this.c3.width/2, this.c3.height - 5);  //X Label
		

        this.ctx3.save();
        this.ctx3.translate(0, 0);
        this.ctx3.rotate(-Math.PI/2);
        this.ctx3.textAlign = "center";
        this.ctx3.fillText("Voltage", -this.c3.height/2, 15);
        this.ctx3.restore();
        ...
        this.ctx3.fillText("1", 30, 40 + 5)
		this.ctx3.fillText("0", 30, this.c3.height/2 + 5)
		this.ctx3.fillText("-1", 30, this.c3.height - 40 + 5)
		...
		while (dashesX <= this.c.width){
			if (labelCounter%5 == 1){
				this.ctx.strokeStyle = 'blue';
				this.ctx3.fillText((Math.round(secondsLabel*100)/100).toString() + " ms", dashesX + 45, this.c3.height - 20)
			} else {
	...
	this.drawData = function(){
		...
		for (var i = -this.analyser.frequencyBinCount/2; i <= this.analyser.frequencyBinCount/2; i++) { //i goes from -numSamples/2 to +numSamples/2
			index = i+this.analyser.frequencyBinCount/2;  // index goes from 0 to numSamples  --> used to find timeDomain info (pulled from fft)

			if (this.streaming == true || this.playingFile == true){


				var height = this.c.height * this.timeDomain[i] / 256;
				var offset = this.c.width * (this.analyser.frequencyBinCount/(this.analyser.frequencyBinCount-1)) * i/this.analyser.frequencyBinCount; //not used

				// analyser.getByteTimeDomainData(timeDomain);

			/* LETS DO SOME MATH!
			Nyquist frequency is one-half of the sample rate 
			frequencyBinCount = one-half fftSize
			frequencyBin = NyquistFreq/frequencyBinCount

			Range = [domFreqIndex * 5.859375 , (domFreqIndex + 1) * 5.859375]
			
			*/		
				
				this.analyser.getByteFrequencyData(this.fData);  //ADDED
				var domFreq = Math.max.apply(Math,this.fData);
				//console.log("DomFreq: " + domFreq);
				var domFreqIndex = this.fData.indexOf(domFreq);
				//console.log("Index: " + domFreqIndex);

				var domFreqLower = domFreqIndex * binSize; // + (5.859375/2);  // Lower threshold
	

				var noteStrings = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
				var noteNum = Math.round(12 * (Math.log( domFreqLower / 440 )/Math.log(2) )) + 69;
				var note = noteStrings[noteNum%12];


				this.ctx3.clearRect(3*this.c3.width/5 + 30, 0 , 170, 35);
				this.ctx3.clearRect(4*this.c3.width/5, 0 , 170, 35);
				this.ctx3.font = "20px Arial";
				this.ctx3.fillText(Math.round(domFreqLower) + " Hz", 3*this.c3.width/5 + 80, 30);
				this.ctx3.fillText( " = " + note ,  4*this.c3.width/5 + 40, 30);



				var xc = i * (this.c.width/this.analyser.frequencyBinCount); // width/fbc = slice width --> increments across x axis 
				// var yc = ui.gain.value * ((timeDomain[index] / 255) - 0.5)*200/(ui.volts.value);
                // var yc = 1 * ((this.timeDomain[index] / 255) - 0.5)*200/(1);
                
                //Scales y values from -100 to +100 * sensitivity value
                var yc = this.graphGain * ((this.timeDomain[index] / 255) - 0.5)*200/(1); //sensitivity * [(scale voltage value 0-1) - .5 [so -.5 to .5] * 200 
