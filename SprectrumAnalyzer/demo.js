...
//var analyser = context.createAnalyser(); //moved from onStream
...
audio.onplay = function(){
  ...
  demo.ui.micInput.value = false;
  $("#micInput-checkbox").attr('checked', false);
  demo.render();
}
// audio.autoplay = true;
     ...
     navigator.mediaDevices.getUserMedia({ audio: true, video: false })
        .then(function(stream) {
          ...
          $("#demo").append($("#labels"));
			    ...
			    demo.attachedCallback();
			    demo.onStream(this.stream);
      ...
      if (e == "micInput"){
        ...
        if (this.ui.micInput.value === false){
          this.mic = false;
          console.log("mic set to " + this.mic);

        } else {
          this.mic = true;
          console.log("mic set to " + this.mic);
          console.log("micInput stream: " + this.stream);
      ...
      if (e == "soundSamples"){
        ...
        audio.src = './' + this.ui.soundSamples.value + ".mp3";
      }
		},
...
// Assumes context is an AudioContext defined outside of this class.
  ...
  updateLabels: function(){
    var domFreqVal = this.getDomFreq();
    this.ui.domFreq.value = domFreqVal;
    this.ui.noteVal.value = this.getNote(domFreqVal);
    
    $("#domFreq-interface input").val(Math.round(this.ui.domFreq.value) + " Hz");
    $("#noteVal-interface input").val(this.ui.noteVal.value); 
  },
  ...
  getDomFreq: function(){
    ...
    var sampleRate = context.sampleRate;     
	  var binSize = (sampleRate*0.5) / this.analyser.frequencyBinCount;
    this.analyser.getByteFrequencyData(fData);  //ADDED
		var domFreq = Math.max.apply(Math,fData);
		var domFreqIndex = fData.indexOf(domFreq);

    return domFreqIndex * binSize;
  ...
  getNote: function(freq){
    var noteStrings = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
		var noteNum = Math.round(12 * (Math.log( freq / 440 )/Math.log(2) )) + 69;
  ...
  onStream: function(stream) {
    //var analyser = context.createAnalyser();
    var input;
    console.log("onStream stream: " + stream);
    console.log("onStream mic value " + this.mic);
    if(this.mic ){
      console.log("in mic true"); 
      input = context.createMediaStreamSource(stream); //this is the issue
    }
    else{
      console.log("in Audio track"); 
